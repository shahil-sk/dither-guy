...
class ImageTab:
    ...
    def build(self) -> toga.Box:
        ...
        for label, handler in [
            ...
            ("Crop",    lambda w: asyncio.ensure_future(self.crop())),
        ]:
    ...
    def open_file(self, window: toga.Window):
        self._window = window
        async def _open(win):
            try:
                path = await win.open_file_dialog(
                    "Open Image",
                    initial_directory=self.last_dir,
                    file_types=["png", "jpg", "jpeg", "bmp", "gif", "tiff", "webp"],
                )
                if path:
                    self._load(str(path), win)
            except Exception:
                pass
        asyncio.ensure_future(_open(window))

    def save_file(self, window: toga.Window):
        self._window = window
        if self.dithered_img is None:
            asyncio.ensure_future(
                window.info_dialog("Nothing to save", "Process an image first.")
            )
            return
        async def _save(win):
            try:
                path = await win.save_file_dialog(
                    "Save Image",
                ...
                await win.error_dialog("Save Error", str(exc))
        asyncio.ensure_future(_save(window))
    ...
    def _load(self, path: str, window: toga.Window = None):
        try:
            ...
            if window:
                asyncio.ensure_future(
                    window.error_dialog("Open Error", str(exc))
                )
    ...
    def rotate_left(self):
        ...
        self.show_status("rotated 90 CCW")
        self._refresh_info()
        self.process()
    ...
    def rotate_right(self):
        ...
        self._push_history()
        self.original_img = self.original_img.rotate(-90, expand=True)
        self.show_status("rotated 90 CW")
        self._refresh_info()
        self.process()
    ...
    async def crop(self):
        ...
        vals = await ask_crop_values(
            self._window,
            self.original_img.width,
            self.original_img.height,
        )
        if vals is None:
            return
        ...
        if l >= x2 or t >= y2:
            await self._window.info_dialog("Crop", "Nothing left after crop.")
            return
        self._push_history()
    ...
    def undo(self):
        ...
        if self._undo_btn:
            self._undo_btn.enabled = bool(self._history)
    ...
    def process(self):
        ...
        try:
            ...
            self.show_status(
                f"{p['method']}  {result.width}x{result.height}"
            )
            if self.original_img:
                ow, oh = self.original_img.size
                if self._info_label:
                    self._info_label.text = (
                        f"{ow}x{oh}  ->  {result.width}x{result.height} px"
                    )
        except Exception as exc:
...
class VideoTab:
    ...
    def build(self) -> toga.Box:
        ...
        self._play_btn = toga.Button(
            "Play",
            on_press=lambda w: self.toggle_play(),
            enabled=False,
            style=Pack(padding=4),
        )

        bar = toga.Box(
            children=[self._play_btn],
    ...
    def open_file(self, window: toga.Window):
        ...
        if not _CV2:
            asyncio.ensure_future(
                window.info_dialog("Missing", "Install opencv-python.")
            )
            return
        async def _open(win):
            try:
    ...
    def _load_video(self, path: str):
        ...
        if self._info_label:
            self._info_label.text = (
                f"{Path(path).name}  {total} frames @ {fps:.1f} fps"
            )
        if self._play_btn:
    ...
    def toggle_play(self):
        ...
        if self._play_btn:
            self._play_btn.text = "Pause" if self.is_playing else "Play"
        if self.is_playing:
    ...
    def save_file(self, window: toga.Window):
        ...
        if not self.video_path:
            asyncio.ensure_future(
                window.info_dialog("No Video", "Load a video first.")
            )
            return
        async def _export(win):
            path = await win.save_file_dialog(
                "Export Video",
        ...
        asyncio.ensure_future(_export(window))
